...
contract TToken is OFTWithFee, Pausable {
  ...
  constructor(
    address _lzEndpoint
  ) OFTWithFee("Tide Token", "TToken", 8, _lzEndpoint) Ownable(msg.sender) {}
  ...
  function _debitFrom(
    ...
    uint256 amount = super._debitFrom(_from, _dstChainId, _toAddress, _amount);
  ...
  function _creditTo(
    ...
    uint256 lastReceivedTimestamp = chainIdToLastReceivedTimestamp[_srcChainId];
    uint256 currTimestamp = block.timestamp;
